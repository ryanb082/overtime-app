...
describe 'navigate' do
  ...
  describe 'index' do
    ...
    it 'has a scope so that only post creators can their post' do
      post1 = Post.create(date: Date.today, rationale: 'some ration of reason', user_id: @user.id)
      post2 = Post.create(date: Date.today, rationale: 'some ration of reason', user_id: @user.id)
  ...
  describe 'edit' do
    before do
      @edit_user = User.create(first_name: 'test', last_name: 'goal', email: 'something@example.com', password: 'asdfasdf', password_confirmation: 'asdfasdf')
      login_as(@edit_user, :scope => :user)
      @edit_post = Post.create(date: Date.today, rationale: 'something', user_id: @edit_user.id)
    end
